...
!****************************************************************************************
  ...
  contains
    ...
    procedure :: rhs_kt_KtEt => rhs_kt_KtEt_DWX
...
subroutine set_bc_DWX(this,ekh,rho,periodic,rank,px)
  ...
  if (isothermalBC.eq.1) then
    do k = 0,k1 
      ...
      botBCvalue     = ekh(1,k)*((this%kt(1,k)**0.5)/walldist(1))**2                
...
subroutine rhs_kt_KtEt_DWX(this,putout,dimpl,rho)
  use mod_param, only : k1,i1,kmax,imax,k,i
  implicit none
  class(DWX_TurbDiffModel) :: this
