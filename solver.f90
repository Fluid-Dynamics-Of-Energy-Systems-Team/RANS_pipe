...
! subroutine solvepois_cr1(rhs,ini,rank,centerBC,hterm)
!   use mod_param, only : k1,i1,kmax,imax,k,i,px,periodic
!   use mod_mesh, only : dzp,dzw,dru,drp,ru,rp
!   implicit none
!   include 'mpif.h'  
...
!   real(8), dimension(imax*kmax*px)  :: pvec_t
!   real(8), dimension(imax,kmax*px)  :: y
!   real(8), dimension(imax,kmax) :: hterm
!   integer ierr,ini,rank, ier
...
! end subroutine  solvepois_cr1


! SUBROUTINE sor(a,b,c,d,e,f,u,jmax,rjac)
!   INTEGER jmax,MAXITS
!   DOUBLE PRECISION rjac,a(jmax,jmax),b(jmax,jmax),c(jmax,jmax),d(jmax,jmax),e(jmax,jmax),f(jmax,jmax),u(jmax,jmax),EPS
...
!   do j=2,jmax-1 !norm has been reduced by a factor EPS.
!     do l=2,jmax-1
